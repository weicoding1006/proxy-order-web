...
#### Scenario: 點擊後台導覽連結
- **WHEN** 使用者點擊後台 Sidebar 中的連結
- **THEN** 頁面切換至對應的 `/admin/*` 路由，主內容區更新為該路由的頁面元件

#### Scenario: 當前後台頁面高亮
- **WHEN** 使用者所在路由與後台某個連結對應
- **THEN** 該連結顯示高亮樣式

### Requirement: 後台 Sidebar 可收合
後台 Sidebar SHALL 提供收合按鈕，切換展開（圖示＋文字）與收合（僅圖示）兩種顯示模式。
